...
{	
	...
	numberOfMaterials = p.numberOfMaterials;
	...
	if(p.theVerts != NULL)
	{
		...
		theMaterials = new aMaterial[numberOfMaterials];
		...
		memcpy(theMaterials, p.theMaterials, sizeof(aMaterial) * numberOfMaterials);
...
//Calculates distance between a point with radius and a plane
float ThreeDModel::calcSidePointPlane(glm::vec3 theBVPoint, float BVRadius, glm::vec3 point, glm::vec3 theFaceNormal) {
	glm::vec3 norm = glm::normalize(theFaceNormal);
	float result = glm::dot(norm, point - theBVPoint) + BVRadius;
	return result;
}

//Checks for octree collisions
bool ThreeDModel::isOctreeCollision(Octree *octree2, glm::mat4 modelMatrix2, glm::vec3 updatedBVPoint) {
	//BASE CASE - IF LEAF
	if (octree2->getLevel() >= MAX_DEPTH) {
		if (octree2 != NULL) {
			//Generate 6 faces
			//Face 1 - Front face
			glm::vec3 point10 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->minY, octree2->maxZ, 1.0f));
			glm::vec3 point11 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->maxY, octree2->maxZ, 1.0f));
			glm::vec3 point12 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->maxZ, 1.0f));

			//Face 2 - Back face
			glm::vec3 point20 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->minZ, 1.0f));
			glm::vec3 point21 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->maxY, octree2->minZ, 1.0f));
			glm::vec3 point22 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->minY, octree2->minZ, 1.0f));

			//Face 3 - Left face
			glm::vec3 point30 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->maxZ, 1.0f));
			glm::vec3 point31 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->maxY, octree2->maxZ, 1.0f));
			glm::vec3 point32 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->minZ, 1.0f));

			//Face 4 - Right face
			glm::vec3 point40 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->minY, octree2->minZ, 1.0f));
			glm::vec3 point41 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->maxY, octree2->minZ, 1.0f));
			glm::vec3 point42 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->minY, octree2->maxZ, 1.0f));

			//Face 5 - Bottom face
			glm::vec3 point50 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->maxZ, 1.0f));
			glm::vec3 point51 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->minY, octree2->minZ, 1.0f));
			glm::vec3 point52 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->minY, octree2->maxZ, 1.0f));

			//Face 6 - Top face
			glm::vec3 point60 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->maxY, octree2->minZ, 1.0f));
			glm::vec3 point61 = glm::vec3(modelMatrix2 * glm::vec4(octree2->minX, octree2->maxY, octree2->maxZ, 1.0f));
			glm::vec3 point62 = glm::vec3(modelMatrix2 * glm::vec4(octree2->maxX, octree2->maxY, octree2->minZ, 1.0f));

			glm::vec3 theFaceNormals[] = {
				glm::cross(point11 - point10, point12 - point10), glm::cross(point21 - point20, point22 - point20), glm::cross(point31 - point30, point32 - point30),
				glm::cross(point41 - point40, point42 - point40), glm::cross(point51 - point50, point52 - point50), glm::cross(point61 - point60, point62 - point60)
			};

			if (calcSidePointPlane(updatedBVPoint, BVRadius[0], point10, theFaceNormals[0]) > 0 &&
				calcSidePointPlane(updatedBVPoint, BVRadius[0], point20, theFaceNormals[1]) > 0 &&
				calcSidePointPlane(updatedBVPoint, BVRadius[0], point30, theFaceNormals[2]) > 0 &&
				calcSidePointPlane(updatedBVPoint, BVRadius[0], point40, theFaceNormals[3]) > 0 &&
				calcSidePointPlane(updatedBVPoint, BVRadius[0], point50, theFaceNormals[4]) > 0 &&
				calcSidePointPlane(updatedBVPoint, BVRadius[0], point60, theFaceNormals[5]) > 0) {
				return true;
			}
		}
		return false;
	...
	//RECURSION THROUGH OCTREE
	else {
		for (int i = 0; i < 8; i++) {
			if (octree2->getChild(i) != NULL) {
				//Generate 6 faces
				//Face 1 - Front face
				glm::vec3 point10 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point11 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->maxY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point12 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));

				//Face 2 - Back face
				glm::vec3 point20 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point21 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->maxY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point22 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->minY, octree2->getChild(i)->minZ, 1.0f));

				//Face 3 - Left face
				glm::vec3 point30 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point31 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->maxY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point32 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->minZ, 1.0f));

				//Face 4 - Right face
				glm::vec3 point40 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->minY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point41 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->maxY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point42 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));

				//Face 5 - Bottom face
				glm::vec3 point50 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point51 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->minY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point52 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->minY, octree2->getChild(i)->maxZ, 1.0f));

				//Face 6 - Top face
				glm::vec3 point60 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->maxY, octree2->getChild(i)->minZ, 1.0f));
				glm::vec3 point61 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->minX, octree2->getChild(i)->maxY, octree2->getChild(i)->maxZ, 1.0f));
				glm::vec3 point62 = glm::vec3(modelMatrix2 * glm::vec4(octree2->getChild(i)->maxX, octree2->getChild(i)->maxY, octree2->getChild(i)->minZ, 1.0f));
				...
				if (calcSidePointPlane(updatedBVPoint, BVRadius[0], point10, theFaceNormals[0]) > 0 &&
					...
					calcSidePointPlane(updatedBVPoint, BVRadius[0], point50, theFaceNormals[4]) > 0 &&
					calcSidePointPlane(updatedBVPoint, BVRadius[0], point60, theFaceNormals[5]) > 0) {
					return isOctreeCollision(octree2->getChild(i), modelMatrix2, updatedBVPoint);
				}
			}
		}
		return false;
...
glm::vec4 ThreeDModel::getLTF(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->minX, octree->maxY, octree->minZ, 1.0f);
}

glm::vec4 ThreeDModel::getLTB(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->minX, octree->maxY, octree->maxZ, 1.0f);
}

glm::vec4 ThreeDModel::getLBF(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->minX, octree->minY, octree->minZ, 1.0f);
}

glm::vec4 ThreeDModel::getLBB(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->minX, octree->minY, octree->maxZ, 1.0f);
}

glm::vec4 ThreeDModel::getRTF(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->maxX, octree->maxY, octree->minZ, 1.0f);
}

glm::vec4 ThreeDModel::getRTB(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->maxX, octree->maxY, octree->maxZ, 1.0f);
}

glm::vec4 ThreeDModel::getRBF(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->maxX, octree->minY, octree->minZ, 1.0f);
}

glm::vec4 ThreeDModel::getRBB(glm::mat4 modelMatrix) {
	return modelMatrix * glm::vec4(octree->maxX, octree->minY, octree->maxZ, 1.0f);
}

glm::vec4 ThreeDModel::getCurrPos(glm::vec4 modelMatrix) {	//Returns current position
	return modelMatrix * glm::vec4();
}
///End of point retrieval

void ThreeDModel::updatePoints(glm::mat4 modelMatrix) {	//Updates all collision points' position on a drone
	for (size_t i = 0; i <= BVPoints->length(); i++) {
		glm::vec4 newPoint = modelMatrix * glm::vec4(BVPoints[i], 1.0f);
		updatedBVPoints[i] = glm::vec3(newPoint.x, newPoint.y, newPoint.z);
	}
}

//Collision reaction
void ThreeDModel::collisionFront(float &speedY, float &speedZ) {
	if (speedZ < 2.0f) {
		speedZ = -1.0f;
	}
	else {
...
void ThreeDModel::collisionRear(float &speedY, float &speedZ) {
	if (speedZ > -1.0f) {
		speedZ = 0.4f;
	}
	else {
...
void ThreeDModel::collisionLeft(float &speedX, float &speedY) {
	if (speedX > -2.0f) {
		speedX = 1.0f;
	}
	else {
...
void ThreeDModel::collisionRight(float &speedX, float &speedY) {
	if (speedX < 2.0f) {
		speedX = -1.0f;
	}
	else {
